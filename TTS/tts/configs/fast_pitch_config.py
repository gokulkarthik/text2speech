...
class FastPitchConfig(BaseTTSConfig):
    ...
    def __post_init__(self):
        ...
        if self.num_languages > 0:
            self.model_args.num_languages = self.num_languages
        
        if self.use_language_embedding:
            self.model_args.use_language_embedding = True
        if self.languages_file:
            self.model_args.languages_file = self.languages_file
